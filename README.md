...
## 当前已落地能力`r`n`r`n- CLI 后端加固：统一事件模型、启动/执行超时、重试与结构化诊断`r`n- 聊天恢复体验：断线重连、resume 语义、会话级运行状态与长消息折叠`r`n- 项目上下文持久化：仓库偏好、最近活动、继续上次工作`r`n- 会话历史治理：搜索、归档/恢复、历史片段引用`r`n- Skill 生命周期升级：启用/禁用、依赖异常诊断、状态持久化`r`n`r`n## 架构文档
