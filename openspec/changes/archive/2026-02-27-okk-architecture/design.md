...
okk 是一个全新项目，目标是构建公司内网的代码仓库知识库平台。当前状态：
...
- 不实现消息平台集成（OpenCowork 的飞书/钉钉/Telegram 插件不在 okk 范围内）
...
**选择**: 使用应用层编排，okk 自己管理 Team 生命周期，通过 AgentRunner 调度多个 CLI 后端子进程

**替代方案**:
- 依赖 CLI 工具内置的 Teams 功能 → 拒绝：无法跨不同 CLI 后端（Claude Code + Codex）协调；且 CLI 工具的 Teams 功能是实验性的，稳定性不可控

**理由**:
- **应用层 Teams**（借鉴 OpenCowork `team-manager`）：okk 创建 Team → 分配任务 → 每个 Sub-Agent 通过 AgentRunner 启动独立的 CLI 子进程 → 汇总结果。优点是可以混合使用 Claude Code 和 Codex 后端，且 okk 完全控制编排逻辑。

### D5: Skills 架构 — 双层结合（MCP + okk Skills）

**选择**: 底层利用 CLI 原生 MCP servers 生态扩展 AI 后端能力，上层 okk 自建少量知识库特有 Skills
...
**理由**:
- **底层 MCP**：Claude Code CLI 和 Codex CLI 都原生支持 MCP。okk 在启动 CLI 子进程前动态生成 MCP 配置文件（Claude Code 用 `.mcp.json`，Codex 用 TOML），注入已启用的 MCP servers。默认集成：
  - `@azure-devops/mcp`（微软官方）— Work Items、Repos、Pipelines、Wiki、PR
  - `mcp-server-git` — Git 历史分析、diff、blame
  - `@modelcontextprotocol/server-sqlite` — 直接查询 okk 知识库数据库
  - Brave/Tavily Search MCP — Web 搜索补充外部文档
- **上层 okk Skills**：知识库特有的轻量级脚本工具，通过 SKILL.md + scripts/ 模式管理，通过系统提示注入给 AI 后端。包括 knowledge-export、repo-stats、code-analysis、dependency-audit。
...
**替代方案**:
- Next.js → 拒绝：okk 是交互密集型 SPA（WebSocket 实时通信、流式渲染），不需要 SSR；且 Next.js 与 Phase 2 Electron 集成更复杂
...
**[Agent Teams 稳定性]** → 使用应用层 Teams（okk 自己控制编排），不依赖 CLI 工具内置的实验性 Teams 功能
